...
const COUNTRIES = [
  ...
  { code: "VC", name: "Saint Vincent", dial: "+1-784" },
...
export function PhoneInput({ value, onChange, error, className }) {
  ...
  return (
    <div className={cn("relative", className)} ref={dropdownRef}>
      <div
        className={cn(
          "flex h-11 w-full rounded-lg border border-input bg-surface text-sm text-foreground",
      ...
      >
        ...
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="flex items-center gap-1.5 border-r border-input px-3 hover:bg-muted/50 transition-colors rounded-l-lg shrink-0"
        >
        ...
        <input
          type="tel"
          value={value}
          onChange={(e) => onChange?.(selected.dial, e.target.value)}
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1 w-72 rounded-xl border border-border bg-surface shadow-xl overflow-hidden">
          {/* Search */}
          <div className="p-2 border-b border-border">
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-8 rounded-md border border-input bg-surface pl-8 pr-3 text-xs focus:outline-none focus:ring-2 focus:ring-ring"
              />
            </div>
          </div>
          ...
          <ul className="max-h-52 overflow-y-auto py-1">
            {filtered.length === 0 ? (
              <li className="px-4 py-3 text-xs text-muted-foreground text-center">No country found</li>
            ) : (
                ...
                <li key={country.code}>
                  <button
                    ...
                    className={cn(
                      ...
                      selected.code === country.code && "bg-muted font-medium"
                    )}
                  >
                    <span className="text-base w-6 shrink-0">{getFlagEmoji(country.code)}</span>
                    <span className="flex-1 truncate">{country.name}</span>
                    <span className="text-xs text-muted-foreground shrink-0">{country.dial}</span>
                  </button>
                </li>
              ))
            )}
